...
def load_model(Configs, VOCAB_SIZE=None, checkpoint=None):
    ...
    if Configs.model.lower() in ["ms-birnnc", "ms-birnn_c"]:
        Model_Configs = Configs.MSBiRNNC
        Model_Configs.vocab_size = VOCAB_SIZE
        model = MS_TL.BiRNN_C(**{**Model_Configs, "class_num": Configs.class_num, })

        # Model_Configs = Configs.MSBiRNNC.MSCNN
        # Model_Configs.vocab_size = VOCAB_SIZE
        # cnn_sub_model = MS_TL.CNN(**{**Model_Configs, "class_num": Configs.class_num, })
        # Model_Configs = Configs.MSBiRNNC.MSBiRNN
        # Model_Configs.vocab_size = VOCAB_SIZE
        # rnn_sub_model = MS_TL.BiRNN(**{**Model_Configs, "class_num": Configs.class_num, })
...
def main(Configs):
    ...
    if Configs.use_processor:
        ...
        for checkpoint in checkpoints:
            ...
            student_model = load_model(Configs, VOCAB_SIZE=tokenizer.vocab_size,checkpoint=checkpoint)
...
if __name__ == '__main__':
    ...
    parser.add_argument("--config_path", type=str, default="./configs/stage3.json")
