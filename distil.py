...
from models import birnn as BiRNN,cnn as CNN,lstmatt as LSTMATT,fcn as FCN, r_bilstm_c as RBC,\
    bilstm_dense as BLSTMDENSE,sesy as SESY, ms as MS_TL
...
def load_teacher_model(Configs, VOCAB_SIZE=None, checkpoint=None):
    # set model

    logger.info("----------------init model-----------------------")

    logger.info("-------------loading teacher model--------------\n\n")
    model_name_or_path = Configs.teacher_model_name_or_path

    if Configs.teacher_model.lower() in ["ts-csw", "cnn"]:
        Model_Configs = Configs.CNN
        model = CNN.BERT_TC.from_pretrained(model_name_or_path,
                                            **{**Model_Configs, "class_num": Configs.class_num, })
    elif Configs.teacher_model.lower() in ["birnn"]:
        Model_Configs = Configs.RNN
        model = BiRNN.BERT_TC.from_pretrained(model_name_or_path,
                                            **{**Model_Configs, "class_num": Configs.class_num, })
    elif Configs.teacher_model.lower() in ["fcn", "fc"]:
        Model_Configs = Configs.FCN
        model = FCN.BERT_TC.from_pretrained(model_name_or_path,**{**Model_Configs, "class_num": Configs.class_num, })
    elif Configs.teacher_model.lower() in ["lstmatt"]:
        Model_Configs = Configs.LSTMATT
        model = LSTMATT.BERT_TC.from_pretrained(model_name_or_path,
                                            **{**Model_Configs, "class_num": Configs.class_num, })
    elif Configs.teacher_model.lower() in ["r-bilstm-c", "r-b-c", "rbc", "rbilstmc"]:
        Model_Configs = Configs.RBiLSTMC
        model = RBC.BERT_TC.from_pretrained(model_name_or_path,
                                                **{**Model_Configs, "class_num": Configs.class_num, })
    elif Configs.teacher_model.lower() in ["bilstmdense", "bilstm-dense", "bilstm_dense", "bi-lstm-dense"]:
        Model_Configs = Configs.BiLSTMDENSE
        model = BLSTMDENSE.BERT_TC.from_pretrained(model_name_or_path,
                                                **{**Model_Configs, "class_num": Configs.class_num, })
    elif Configs.teacher_model.lower() in ["sesy"]:
        Model_Configs = Configs.SESY
        model = SESY.BERT_TC.from_pretrained(model_name_or_path,
                                                **{**Model_Configs, "class_num": Configs.class_num, })
    elif Configs.teacher_model.lower() in ["ft-bert", "finetune-bert", "ftbert", "finetunebert"]:
        Model_Configs = Configs.FineTuneBERT
        model = MS_TL.MyBert.from_pretrained(model_name_or_path,
                                                **{**Model_Configs, "class_num": Configs.class_num, })
    else:
        logger.error("no such model, exit")
        exit()

    logger.info("Teacher Model Configs")
    logger.info(json.dumps({**{"MODEL_TYPE": Configs.teacher_model}, **Model_Configs, }))
    model = model.to(Configs.device)
    return model


def load_student_model(Configs, VOCAB_SIZE=None, checkpoint=None):

    logger.info("----------------init model-----------------------")
    if Configs.student_model.lower() in ["ts-csw", "cnn"]:
        Model_Configs = Configs.CNN
        Model_Configs.vocab_size = VOCAB_SIZE
        model = CNN.TC(**{**Model_Configs, "class_num": Configs.class_num, })
    elif Configs.student_model.lower() in ["birnn"]:
        Model_Configs = Configs.RNN
        Model_Configs.vocab_size = VOCAB_SIZE
        model = BiRNN.TC(**{**Model_Configs, "class_num": Configs.class_num})
    elif Configs.student_model.lower() in ["fcn", "fc"]:
        Model_Configs = Configs.FCN
        Model_Configs.vocab_size = VOCAB_SIZE
        model = FCN.TC(**{**Model_Configs, "class_num": Configs.class_num, })
    elif Configs.student_model.lower() in ["lstmatt"]:
        Model_Configs = Configs.LSTMATT
        Model_Configs.vocab_size = VOCAB_SIZE
        model = LSTMATT.TC(**{**Model_Configs, "class_num": Configs.class_num, })
    elif Configs.student_model.lower() in ["r-bilstm-c", "r-b-c", "rbc", "rbilstmc"]:
        Model_Configs = Configs.RBiLSTMC
        Model_Configs.vocab_size = VOCAB_SIZE
        model = RBC.TC(**{**Model_Configs, "class_num": Configs.class_num, })
    elif Configs.student_model.lower() in ["bilstmdense", "bilstm-dense", "bilstm_dense", "bi-lstm-dense"]:
        Model_Configs = Configs.BiLSTMDENSE
        Model_Configs.vocab_size = VOCAB_SIZE
        model = BLSTMDENSE.TC(**{**Model_Configs, "class_num": Configs.class_num, })
    elif Configs.student_model.lower() in ["sesy"]:
        Model_Configs = Configs.SESY
        Model_Configs.vocab_size = VOCAB_SIZE
        model = SESY.TC(**{**Model_Configs, "class_num": Configs.class_num, })
    elif Configs.student_model.lower() in ["ms-cnn", "mscnn", "multistage-cnn"]:
        Model_Configs = Configs.MSCNN
        Model_Configs.vocab_size = VOCAB_SIZE
        model = MS_TL.CNN(**{**Model_Configs, "class_num": Configs.class_num, })
    elif Configs.student_model.lower() in ["ms-rnn", "msrnn", "multistage-rnn","ms-birnn", "msbirnn", "multistage-birnn"]:
        Model_Configs = Configs.MSBiRNN
        Model_Configs.vocab_size = VOCAB_SIZE
        model = MS_TL.BiRNN(**{**Model_Configs, "class_num": Configs.class_num, })
    ...
    logger.info("Student Model Configs")
    logger.info(json.dumps({**{"MODEL_TYPE": Configs.student_model}, **Model_Configs, }))
    model = model.to(Configs.device)
    return model


def train(teacher_model=None, model=None, Configs=None, tokenizer=None):
    ...
    if teacher_model is not None:
        if Training_Configs.teacher_criteration == "KLDivLoss":
            teaching_criterion = torch.nn.KLDivLoss(reduction='batchmean')
    ...
    for epoch_n in train_iterator:
        ...
        for step, batch in enumerate(epoch_iterator):
            ...
            if teacher_model is not None:
                ...
                outputs_S = torch.nn.functional.log_softmax(student_logits / Training_Configs.distil_T, dim=1)
                outputs_T = torch.nn.functional.softmax(teacher_logits / Training_Configs.distil_T, dim=1)
                teaching_loss = teaching_criterion(outputs_S, outputs_T)*Training_Configs.distil_T*Training_Configs.distil_T
                loss = student_loss*(1-Training_Configs.distil_alpha) + teaching_loss*Training_Configs.distil_alpha
            else:
...
def main(Configs):
    ...
    if Configs.use_processor:
        ...
        if not Dataset_Configs.resplit and os.path.exists(Dataset_Configs.csv_dir) and \
            ...
            train_texts,val_texts,train_labels,val_labels = train_test_split(train_texts, train_labels, train_size=Dataset_Configs.split_ratio)
    ...
    teacher_model = load_teacher_model(Configs, VOCAB_SIZE=VOCAB_SIZE)
    student_model =  load_student_model(Configs, VOCAB_SIZE=VOCAB_SIZE)

    logger.info("--------------start training--------------------")

    if Configs.use_processor:
        # train_dataset = load_and_cache_examples(Dataset_Configs, Configs.task_name, tokenizer)  # , evaluate=False)
        global_step, tr_loss = train(teacher_model, student_model, Configs, tokenizer)
        ...
        for checkpoint in checkpoints:
            ...
            student_model = load_student_model(Configs, VOCAB_SIZE=tokenizer.vocab_size,checkpoint=checkpoint)
            ...
            result, preds, ex_ids = evaluate(student_model, tokenizer, Configs, Configs.task_name, split="test", prefix=prefix)
...
if __name__ == '__main__':
    ...
    parser.add_argument("--config_path", type=str, default="./configs/stage2.json")
